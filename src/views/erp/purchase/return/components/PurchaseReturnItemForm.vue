      ...
      <el-table-column label="批次选择" min-width="200" v-if="showBatchSelection">
        <template #default="{ row, $index }">
          ...
          >
            ...
            >
              <el-option
                v-for="item in row.availableStocks || []"
                :key="item.purchaseInItemId"
                :label="`${item.batchName || '批次' + item.purchaseInItemId} (库存:${item.stockCount})`"
        ...
        <template #default="{ row, $index }">
          ...
          >
            <el-button @click="openUniqueCodeSelectionDialog(row)" type="primary" link>
              {{ row.uniqueCodes && row.uniqueCodes.length > 0 ? `已选${row.uniqueCodes.length}个` : '选择一物一码' }}
            </el-button>
          </el-form-item>
...
const onChangeBatch = (batchId: number, row: any) => {
  ...
  if (row.batchWarehouses.length === 1) {
    ...
    row.maxReturnCount = row.batchWarehouses[0].stockCount
    
    // 如果当前退货数量超过库存，自动调整为库存数量
  ...
  if (row.manageType === 20 || row.manageType === 30) {
    if (row.batchWarehouses && row.batchWarehouses.length > 0) {
      ...
      if (selectedStock) {
        ...
        row.maxReturnCount = selectedStock.stockCount
...
const openUniqueCodeSelectionDialog = async (row: any) => {
  ...
  try {
    ...
    const result = await UniqueCodeApi.getInStockUniqueCodeList({
...
const handleCrossWarehouseSelection = (warehouseIds: number[]) => {
  ...
  warehouseGroups.forEach((uniqueCodes, warehouseId) => {
    ...
    const newRow = {
      ...
      maxReturnCount: warehouseStock?.stockCount || 0,
      // 重新计算价格
...
const confirmUniqueCodeSelection = () => {
  ...
  if (warehouseIds.length > 1) {
    ...
    message.success(`已选择${selectedUniqueCodes.value.length}个一物一码，自动拆分为${warehouseIds.length}个退货项`)
  } else {
    // 单仓库选择：直接保存
    ...
    if (!currentRow.value.warehouseId && warehouseIds.length === 1) {
      ...
      if (selectedStock) {
        ...
        currentRow.value.maxReturnCount = selectedStock.stockCount
      }
    }
