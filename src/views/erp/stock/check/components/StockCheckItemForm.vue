<template>
  ...
  >
    <div>
      <p class="mb-2">
        ...
        | 需要输入：<strong>{{ Math.floor(currentEditRow?.actualCount || 0) }}</strong> 个
      </p>
  ...
  >
    <div>
      <p class="text-gray-500 text-sm mb-2">
        提示：系统将自动根据一物一码查询产品和批次信息，并按产品+仓库+批次自动拆分盘点项
      </p>
      <el-input
...
const currentEditRow = ref<{ productName?: string; actualCount?: number; uniqueCodes?: string[] } | null>(null)
...
const getSummaries = (param: SummaryMethodProps) => {
  ...
  columns.forEach((column, index) => {
    ...
    if (['count', 'totalPrice', 'actualCount', 'stockCount'].includes(column.property)) {
      const sum = getSumValue(data.map((item) => Number(item[column.property])))
      sums[index] =
        column.property === 'count' || column.property === 'actualCount' || column.property === 'stockCount'
...
const buildRowKey = (row: any) => {
  ...
  return `${manageType}__${productId}__${warehouseId}__${batchId}`
}
...
const setStockCount = async (row) => {
  ...
  // 初次加载时，实际库存默认等于账面库存
  if (row.actualCount == null || row.actualCount === 0) {
...
const confirmBatchUniqueCodeInput = async () => {
  ...
  try {
    ...
    // 调用后端接口解析一物一码
    const result = await StockCheckApi.parseUniqueCodesForCheck({
      codes: uniqueCodes,
      deptId: props.deptId!
    })

    if (!result || result.length === 0) {
      message.error('未能解析出有效的盘点项')
      return
    }
    ...
    result.forEach((item: any) => {
      ...
      const row: any = {
        ...
        actualCount: Number(item.uniqueCodes?.length) || 0,
        count: undefined,
        totalPrice: undefined,
        remark: undefined,
      ...
      if (target) {
        ...
        if (row.manageType === 30) {
          ...
          if (appended > 0) {
            ...
            target.actualCount = target.uniqueCodes.length
            target.count = target.actualCount - target.stockCount
            changed = true
          }
        }

        if (changed) {
          target.totalPrice = erpPriceMultiply(target.productPrice ?? row.productPrice ?? 0, target.count || 0)
